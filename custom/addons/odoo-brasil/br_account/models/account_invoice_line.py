...
class AccountInvoiceLine(models.Model):
    ...
    def _set_extimated_taxes(self, price):
        ...
        if self.product_type == 'service':
            ...
            vtax_id = self.env['account.tax'].search([('domain','=','outros')])
            vtax = vtax_id.amount
            if vtax == 0:
                self.tributos_estimados_federais = price * (federal / 100)
                self.tributos_estimados_estaduais = price * (ncm.estadual_imposto / 100)
                self.tributos_estimados_municipais = price * (ncm.municipal_imposto / 100)
            else:
                self.tributos_estimados_federais = price * (vtax/100)
                self.tributos_estimados_estaduais = 0
                self.tributos_estimados_municipais = 0
